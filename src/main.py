...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "mode", choices=["index", "chat", "fetch-notion"], help="operation mode")
    parser.add_argument("--pdf_dir", default="data/chapters/",
                        help="directory containing PDF files")
    parser.add_argument("--index_prefix", default="textbook_index",
                        help="prefix for generated index files")
    parser.add_argument("--model_path", help="path to generation model")
    parser.add_argument("--system_prompt_mode",
                        choices=["baseline", "tutor", "concise", "detailed"], default="baseline")
    ...
    notion_group.add_argument(
        "--notion_token", help="Notion integration token")
    notion_group.add_argument(
        "--notion_page_id", help="Root Notion page ID to fetch")
    notion_group.add_argument(
        "--notion_recursive", action="store_true", help="Fetch child pages recursively")
...
def run_fetch_notion_mode(args: argparse.Namespace):
    import os
    from src.notion_processing.fetch_notion import spool_notion_to_disk

    token = args.notion_token or os.getenv("NOTION_TOKEN")
    if not token:
        print("ERROR: Notion token must be provided via --notion_token or NOTION_TOKEN environment variable.", file=sys.stderr)
        sys.exit(1)

    page_id = args.notion_page_id or os.getenv("NOTION_ROOT_PAGE_ID")
    if not page_id:
        print("ERROR: Notion root page ID must be provided via --notion_page_id or NOTION_ROOT_PAGE_ID environment variable.", file=sys.stderr)
        sys.exit(1)

    spool_notion_to_disk(token=token, root_page_id=page_id,
...
def get_answer(
    ...
    if golden_chunks and cfg.use_golden_chunks:
        ...
        if is_test_mode:
            ...
            faiss_ranked = sorted(faiss_scores.keys(),
                                  key=lambda i: faiss_scores[i], reverse=True)
            bm25_ranked = sorted(bm25_scores.keys(),
                                 key=lambda i: bm25_scores[i], reverse=True)
            index_ranked = sorted(index_scores.keys(),
                                  key=lambda i: index_scores[i], reverse=True)

            faiss_ranks = {idx: rank + 1 for rank,
                           idx in enumerate(faiss_ranked)}
            bm25_ranks = {idx: rank + 1 for rank,
                          idx in enumerate(bm25_ranked)}
            index_ranks = {idx: rank + 1 for rank,
                           idx in enumerate(index_ranked)}
        ...
        ranked_chunks = rerank(question, ranked_chunks,
                               mode=cfg.rerank_mode, top_n=cfg.rerank_top_k)
    ...
    if is_test_mode:
        ...
        logger.save_chat_log(
            ...
            ordered_scores=scores[:len(
                topk_idxs)] if 'scores' in locals() else [],
            chat_request_params={
...
def run_chat_session(args: argparse.Namespace, cfg: RAGConfig):
    ...
    try:
        ...
        print(
            f"Loaded {len(chunks)} chunks and {len(sources)} sources from artifacts.")
        retrievers = [FAISSRetriever(
            faiss_idx, cfg.embed_model), BM25Retriever(bm25_idx)]
        if cfg.ranker_weights.get("index_keywords", 0) > 0:
            retrievers.append(IndexKeywordRetriever(
                cfg.extracted_index_path, cfg.page_to_chunk_map_path))

        ranker = EnsembleRanker(ensemble_method=cfg.ensemble_method,
                                weights=cfg.ranker_weights, rrf_k=int(cfg.rrf_k))
    ...
    while True:
        # Debug print to trace chat history
        ...
        try:
            ...
            if cfg.enable_history and chat_history:
                try:
                    effective_q = contextualize_query(
                        q, chat_history, cfg.gen_model)
                    ...
                    # Debug print to trace contextualization
                    print(f"Contextualized Query: {effective_q}")
                except Exception as e:
                    print(
                        f"Warning: Failed to contextualize query: {e}. Using original query.")
                    effective_q = q
            ...
            ans = get_answer(effective_q, cfg, args, logger, console,
                             artifacts=artifacts, additional_log_info=additional_log_info)
...
def main():
    ...
    elif args.mode == "fetch-notion":
        run_fetch_notion_mode(args)
