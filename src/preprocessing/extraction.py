...
def extract_sections_from_markdown(
    ...
    heading_pattern = r'(?=^#+ .*)'  # match any heading level
...
def extract_index_with_range_expansion(text_content):
    ...
    text_content = re.sub(r'^\d+\s+Index\s*$', '',
                          text_content, flags=re.MULTILINE)
    text_content = re.sub(r'^Index\s+\d+\s*$', '',
                          text_content, flags=re.MULTILINE)
...
def convert_and_save_with_page_numbers(input_file_path, output_file_path):
    ...
    try:
        ...
        print(
            f"Error writing to file {output_file_path}: {e}", file=sys.stderr)
