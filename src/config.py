...
class RAGConfig:
    ...
    ranker_weights: Dict[str, float] = field(
        default_factory=lambda: {"faiss": 1.0,
                                 "bm25": 0.0, "index_keywords": 0.0}
    )
    rerank_mode: str = ""
    rerank_top_k: int = 5
    ...
    def __post_init__(self):
        ...
        if self.ensemble_method.lower() in {"linear", "weighted"}:
            ...
            self.ranker_weights = {k: v/s for k,
                                   v in self.ranker_weights.items()}
    ...
    def get_chunk_config(self) -> ChunkConfig:
        ...
        if self.chunk_mode == "recursive_sections":
            ...
            raise ValueError(
                f"Unknown chunk_mode: {self.chunk_mode}. Supported: recursive_sections")
    ...
    def get_chunk_strategy(self) -> ChunkStrategy:
        ...
        raise ValueError(
            f"Unknown chunk config type: {self.chunk_config.__class__.__name__}")
    ...
    def get_config_state(self) -> None:
        ...
        # remove chunk_config to avoid serialization issues
