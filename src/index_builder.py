...
DEFAULT_EXCLUSION_KEYWORDS = ['questions',
...
def build_index(
    ...
    *,
        ...
        for i, c in enumerate(sections):
            ...
            for sub_chunk_id, sub_chunk in enumerate(sub_chunks):
                ...
                if fragments[0].strip():
                    page_to_chunk_ids.setdefault(current_page, set()).add(
                        total_chunks+sub_chunk_id)
                ...
                for j in range(1, len(fragments), 2):
                    try:
                        ...
                        if fragments[j+1].strip():
                            page_to_chunk_ids.setdefault(new_page, set()).add(
                                total_chunks + sub_chunk_id)
    ...
    print(
        f"Embedding {len(all_chunks):,} chunks with {pathlib.Path(embedding_model_path).stem} ...")
